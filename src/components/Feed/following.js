...
import { View, Text, TouchableOpacity, TouchableWithoutFeedback, ScrollView, RefreshControl } from 'react-native';
...
export default () =>
{
    const { FollowList, isRefreshingFollow } = useSelector((state) => state.models);
    // 
    const dispatch = useDispatch();
    const navigation =  useNavigation();
    // 
    useEffect(()=>{
        SEND('tweet/following',{});
      },[]);
    // 
    const handleComment = (tweetInfo) => {
        navigation.navigate('feedInfo', tweetInfo)
    };
    // 
    ...
    return <View style={styles.h100}>
        <Tabs />
        <ScrollView refreshControl={<RefreshControl refreshing={isRefreshingFollow || false} onRefresh={onRefresh} />}>
            {FollowList&&FollowList.map((v,k)=>(
                <View key={k} style={styles.tweetContainer}>
                    <View style={{flexDirection:'row', columnGap:10}}>
                        ...
                        <View style={{flex:1, flexBasis:'auto'}}>
                            ...
                            <View style={{flexDirection: 'row', alignItems:'center', columnGap:10, paddingTop:10}}>
                                ...
                                <TouchableOpacity style={styles.actionButton} onPress={()=> handleComment({tweetInfo:v})}>
                                    <FontAwesome5 name="comment" size={20} color="black" />
                                    <Text style={styles.actionButtonText}> {v[9]}</Text>
                                </TouchableOpacity>
                            </View>
